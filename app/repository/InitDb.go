...
// 初始化数据库
func RegisterDb() {
	qbs.Register("mysql", _DBSPEC, _DBNAME, qbs.NewMysql())
	// 创建数据库
	...
	m.CreateTableIfNotExists(new(models.SourceLog))
	m.CreateTableIfNotExists(new(models.FilterLog))
	m.CreateTableIfNotExists(new(models.TargetLog))
